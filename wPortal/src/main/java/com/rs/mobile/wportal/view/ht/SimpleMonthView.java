...
class SimpleMonthView extends View {
	...
	public SimpleMonthView(Context context, TypedArray typedArray, DayPickerView.DataModel dataModel) {
		...
		mYearMonthTextColor = typedArray.getColor(R.styleable.DayPickerView_colorYearMonthText,
				resources.getColor(R.color.normal_day));
		mWeekTextColor = typedArray.getColor(R.styleable.DayPickerView_colorWeekText,
				resources.getColor(R.color.normal_day));
		// mDayTextColor =
		// typedArray.getColor(R.styleable.DayPickerView_colorDayName,
		// resources.getColor(R.color.normal_day));
		mDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorNormalDayText,
				resources.getColor(R.color.normal_day));
		mPreviousDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorPreviousDayText,
				resources.getColor(R.color.normal_day));
		mSelectedDaysBgColor = typedArray.getColor(R.styleable.DayPickerView_colorSelectedDayBackground,
				resources.getColor(R.color.selected_day_background));
		mSelectedDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorSelectedDayText,
		...
		mBusyDaysTextColor = typedArray.getColor(R.styleable.DayPickerView_colorBusyDaysText,
				resources.getColor(R.color.normal_day));
		mInValidDaysTextColor = typedArray.getColor(R.styleable.DayPickerView_colorInValidDaysText,
				resources.getColor(R.color.normal_day));
		// mDrawRect =
		// typedArray.getBoolean(R.styleable.DayPickerView_drawRoundRect, true);
		...
		TAG_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeTag,
				resources.getDimensionPixelSize(R.dimen.text_size_tag));
		YEAR_MONTH_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeYearMonth,
				resources.getDimensionPixelSize(R.dimen.text_size_month));
		WEEK_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeWeek,
		...
		DAY_SELECTED_RECT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_selectedDayRadius,
	...
	private void drawMonthDayLabels(Canvas canvas) {
		...
		for (int i = 0; i < mNumDays; i++) {
			...
			canvas.drawText(mDateFormatSymbols.getShortWeekdays()[mDayLabelCalendar.get(Calendar.DAY_OF_WEEK)]
					.toUpperCase(Locale.getDefault()), x, y, mWeekTextPaint);
		}
	}
	...
	protected void drawMonthCell(Canvas canvas) {
		...
		while (day <= mNumCells) {
			...
			if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
				...
				if (isToady) {
					canvas.drawText("今天", x, getTextYCenter(mDayTextPaint, y - DAY_SELECTED_RECT_SIZE / 2),
							mDayTextPaint);
				}
			}
			...
			if (cellCalendar.after(mStartDate) && cellCalendar.before(mEndDate)) {
				...
				// DAY_SELECTED_RECT_SIZE / 2, mDayTextPaint);
			}
			...
			for (SimpleMonthAdapter.CalendarDay calendarDay : mBusyDays) {
				if (cellCalendar.equals(calendarDay) && !isPrevDay) {
					...
					// DAY_SELECTED_RECT_SIZE,
					// x + DAY_SELECTED_RECT_SIZE, (y -
					// MINI_DAY_NUMBER_TEXT_SIZE / 3) + DAY_SELECTED_RECT_SIZE);

					// 选择了入住和退房日期，退房日期等于mNearestDay的情况
					if (mStartDate != null && mEndDate != null && mNearestDay != null && mEndDate.equals(mNearestDay)
							&& mEndDate.equals(calendarDay)) {

					} else {
						// 选择了入住日期，没有选择退房日期，mNearestDay变为可选且不变灰色
						if (mStartDate != null && mEndDate == null && mNearestDay != null
								&& cellCalendar.equals(mNearestDay)) {
							mDayTextPaint.setColor(mDayTextColor);
						} else {
						...
						canvas.drawText("已租", x, getTextYCenter(mBusyDayBgPaint, y + DAY_SELECTED_RECT_SIZE / 2),
								mDayTextPaint);
					}
					canvas.drawText(String.format("%d", day), x,
							getTextYCenter(mTagTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
				}
			}
			...
			for (SimpleMonthAdapter.CalendarDay calendarDay : mInvalidDays) {
				...
				if (cellCalendar.equals(calendarDay) && !isPrevDay) {
					...
					if (mStartDate != null && mEndDate != null && mNearestDay != null && mEndDate.equals(mNearestDay)
						...
						canvas.drawText("禁用", x, getTextYCenter(mInValidDayBgPaint, y + DAY_SELECTED_RECT_SIZE / 2),
								mDayTextPaint);
					}
					canvas.drawText(String.format("%d", day), x,
							getTextYCenter(mTagTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
			...
			if (mStartDate != null && mEndDate == null && !mStartDate.equals(mEndDate) && !isInvalidDays
					&& !isBusyDay) {
				if (cellCalendar.before(mStartDate) || (mNearestDay != null && cellCalendar.after(mNearestDay))) {
					// RectF rectF = new RectF(x - DAY_SELECTED_RECT_SIZE, (y -
					// MINI_DAY_NUMBER_TEXT_SIZE / 3) - DAY_SELECTED_RECT_SIZE,
					// x + DAY_SELECTED_RECT_SIZE, (y -
					// MINI_DAY_NUMBER_TEXT_SIZE / 3) + DAY_SELECTED_RECT_SIZE);
					// canvas.drawRoundRect(rectF, 10.0f, 10.0f,
			...
			if (!isPrevDay && !isInvalidDays && !isBusyDay && !isBeginDay && !isLastDay) {
				...
				for (SimpleMonthAdapter.CalendarDay calendarDay : mCalendarTags) {
					if (cellCalendar.equals(calendarDay)) {
						isCalendarTag = true;
						canvas.drawText(calendarDay.tag, x,
								getTextYCenter(mTagTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mTagTextPaint);
					}
				}
				if (!isCalendarTag) {
					canvas.drawText(mDefTag, x, getTextYCenter(mTagTextPaint, y + DAY_SELECTED_RECT_SIZE / 2),
							mTagTextPaint);
				}
			}
	...
	protected void initView() {
		...
		// mSelectedDayTextPaint.setTypeface(Typeface.create(mDayOfWeekTypeface,
		// Typeface.NORMAL));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(MeasureSpec.getSize(widthMeasureSpec),
				mRowHeight * mNumRows + MONTH_HEADER_SIZE + ROW_SEPARATOR);
	}
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			...
			for (SimpleMonthAdapter.CalendarDay day : mInvalidDays) {
				...
				if (calendarDay.equals(day)
						&& !(mEndDate == null && mNearestDay != null && calendarDay.equals(mNearestDay))) {
					return true;
				}
			}
	...
	private void drawDayBg(Canvas canvas, int x, int y, Paint paint) {
		RectF rectF = new RectF(x - DAY_SELECTED_RECT_SIZE, y - DAY_SELECTED_RECT_SIZE, x + DAY_SELECTED_RECT_SIZE,
				y + DAY_SELECTED_RECT_SIZE);
