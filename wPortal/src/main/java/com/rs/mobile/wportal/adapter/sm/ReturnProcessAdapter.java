...
public class ReturnProcessAdapter extends BaseAdapter {
	...
	public ReturnProcessAdapter(JSONArray arr, Context context) {
		super();
		this.arr = arr;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			if (new JSONObject(getItem(position).toString()).get(C.KEY_JSON_FM_HASCURRENT).toString().equals("true")) {
			...
			text_time.setText(new JSONObject(getItem(position).toString()).get(C.KEY_JSON_FM_REFDATE).toString());
			text_detail.setText(new JSONObject(getItem(position).toString()).get(C.KEY_JSON_FM_REFLOCATION).toString());
		} catch (Exception e) {
