package com.rs.mobile.wportal.takephoto;
 ...
 */
public class PickOrTakeImageActivity extends BaseActivity implements View.OnClickListener,
	...
	protected void initView() {
		...
		// animation = new TranslateAnimation(0, 0, 0, -CommonUtil.dip2px(this,
		// 300));
		// reverseanimation = new TranslateAnimation(0, 0,
		// -CommonUtil.dip2px(this, 300), 0);
		if (Build.VERSION.SDK_INT < 11) {
			...
			animation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
				...
				public void onAnimationUpdate(ValueAnimator valueAnimator) {
					...
					rl_choose_directory
							.setAlpha(1 - Math.abs(value / CommonUtil.dip2px(PickOrTakeImageActivity.this, 400)));
		...
		((TextView) findViewById(R.id.tv_title)).setText(getString(R.string.common_text114));
		findViewById(R.id.tv_title).setOnClickListener(new OnClickListener() {
	...
	protected void initData() {
		...
		// 计算每张图片应该显示的宽度
		perWidth = (((WindowManager) (A.getBaseAppContext().getSystemService(Context.WINDOW_SERVICE)))
				.getDefaultDisplay().getWidth() - CommonUtil.dip2px(this, 4)) / 3;
		...
		if (picNums == 1) {
			...
			btn_choose_finish.setText(getString(R.string.complete));
		}
	}
	...
	private void getAllImages() {
		...
		// 需要弹出dialog让用户手动赋予权限
			...
			ActivityCompat.requestPermissions(PickOrTakeImageActivity.this,
	...
	private void startGetImageThread() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				// null, MediaStore.Images.Media.MIME_TYPE + "=\"image/jpeg\" or
				// "
				...
				if (cursor != null) {
					...
					while (cursor.moveToNext()) {
						...
						try {
							singleImageModel.date = Long.parseLong(
									cursor.getString(cursor.getColumnIndex(MediaStore.Images.Media.DATE_MODIFIED)));

							singleImageModel.id = Long.parseLong(
									cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Images.Media._ID)));
							if (new File(singleImageModel.path).exists()) {
								// System.err.println("getBitmapFromPath: file
								// not exists");
		...
		if (requestCode == CODE_FOR_WRITE_PERMISSION) {
			if (permissions[0].equals(Manifest.permission.WRITE_EXTERNAL_STORAGE)
				...
				if (!ActivityCompat.shouldShowRequestPermissionRationale(this,
						Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
					AlertDialog dialog = new AlertDialog.Builder(this).setMessage(getString(R.string.common_text115))
							.setPositiveButton(getString(R.string.common_text003),
	...
	private void reloadDataByChooseDirectory() {
		if (currentShowPosition == -1) {
			...
			tv_choose_image_directory
					.setText(new File(imageDirectories.get(currentShowPosition).directoryPath).getName());
		}
		// 去除当前正在加载的所有图片，重新开始
		AlbumBitmapCacheHelper.getInstance().removeAllThreads();
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				for (int i = 0; i < picklist.size(); i++) {
					if (picklist.get(i) != null || !picklist.get(i).equals("")) {
						arrayList.add(ImageUtil.savePublishPicture(picklist.get(i)));
					}
	...
	private class GridViewAdapter extends BaseAdapter {
		...
		public View getView(int i, View view, ViewGroup viewGroup) {
			...
			Bitmap bitmap = AlbumBitmapCacheHelper.getInstance().getBitmap(path, perWidth, perWidth,
	...
	private void takePic() {
		...
		d("Log+_____________" + CommonUtil.getDataPath().toString());
		tempPath = CommonUtil.getDataPath() + System.currentTimeMillis() + ".jpg";
	...
	private class ListviewAdapter extends BaseAdapter {
		...
		public View getView(int i, View view, ViewGroup viewGroup) {
			...
			if (getItemViewType(i) == 0) {
				...
				holder.tv_directory_nums.setText(size + getString(R.string.common_text119));
				// 获取第0个位置的图片，即第一张图片展示
				...
				holder.tv_directory_nums.setText(
						imageDirectories.get(i - 1).images.getImageCounts() + getString(R.string.common_text119));
				if (currentShowPosition == i - 1) {
					holder.iv_directory_check.setTag("picked");
					holder.iv_directory_check.setVisibility(View.VISIBLE);
				} else {
			...
			if (holder.iv_directory_pic.getTag() != null) {
				AlbumBitmapCacheHelper.getInstance()
						.removePathFromShowlist((String) (holder.iv_directory_pic.getTag()));
			}
			AlbumBitmapCacheHelper.getInstance().addPathToShowlist(path);
			if (getItemViewType(i) == 0) {
			...
			Bitmap bitmap = AlbumBitmapCacheHelper.getInstance().getBitmap(path, 225, 225,
					new AlbumBitmapCacheHelper.ILoadImageCallback() {
						@Override
	...
	private void toggleImageDirectoryModelStateFromMapById(int position) {
		...
		if (currentShowPosition == -1) {
			...
			for (SingleImageModel model : allImages) {
				if (model.path
						.equalsIgnoreCase(imageDirectories.get(currentShowPosition).images.getImagePath(position)))
				...
				if (getImageDirectoryModelStateFromMapById(position)) {
					if (currentPicNums == picNums) {
						...
						Toast.makeText(PickOrTakeImageActivity.this,
								String.format(getString(R.string.choose_pic_num_out_of_index), picNums),
					...
					btn_choose_finish.setText(
							String.format(getString(R.string.choose_pic_finish_with_num), currentPicNums, picNums));
				} else {
				...
				for (String path : temp) {
					if (!picklist.contains(path)) {
						...
						if (v != null) {
							...
							((ImageView) ((ViewGroup) (v.getParent())).findViewById(R.id.iv_pick_or_not))
									.setImageResource(R.drawable.image_choose);
						}
				...
				for (String path : picklist) {
					if (!temp.contains(path)) {
						...
						if (v != null) {
							...
							((ImageView) ((ViewGroup) (v.getParent())).findViewById(R.id.iv_pick_or_not))
									.setImageResource(R.drawable.image_not_chose);
						}
						currentPicNums--;
	...
	private void returnDataAndClose() {
		...
		// if (currentPicNums == 0) {
		// Toast.makeText(this, getString(R.string.not_choose_any_pick),
		// Toast.LENGTH_SHORT).show();
		// return;
		// }
		...
		// dialog.show();
		// t(picklist.toString());
		D.showProgressDialog(PickOrTakeImageActivity.this, "", true);
