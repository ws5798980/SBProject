...
public class SmAddAddressActivity extends BaseActivity {
	...
	private TextView tv_postNumber, tv_addr01_txt;

	private StateButton tv_addr_searche;
...
//			text_next.setText(getIntent().getStringExtra(C.KEY_JSON_FM_LOCATION));
	...
	private void initView() {
		...
		try {
			...
			tv_addr_searche = (StateButton) findViewById(R.id.addr_searche);
			tv_addr_searche.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					Intent intent = new Intent(SmAddAddressActivity.this, kfme_post_search.class);
					startActivityForResult(intent, 1000);
				}
			});

			tv_postNumber = (TextView)findViewById(R.id.postNumber);
...
//										+ S.getShare(SmAddAddressActivity.this, C.KEY_JSON_CUSTOM_CODE, "") + "|" + Util.getDeviceId(getApplicationContext());
			params.put(C.KEY_JSON_TOKEN, S.get(SmAddAddressActivity.this, C.KEY_JSON_TOKEN));
			params.put(C.KEY_JSON_FM_DELIVER_ID, username);
			params.put(C.KEY_JSON_FM_DEFAULT_ADD, checkBox.isChecked() + "");
			...
			OkHttpHelper okHttpHelper = new OkHttpHelper(SmAddAddressActivity.this);
