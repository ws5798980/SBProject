...
public final class EncodingHandler {
 ...
 public static Bitmap CreateOneDCode2(Context context,String content) throws WriterException {  
	        // 生成一维条码,编码时指定大小,不要生成了图片以后再进行缩放,这样会模糊导致识别失败  
	        BitMatrix matrix = new MultiFormatWriter().encode(content,  
 ...
 public static Bitmap CreateOneDCode3(Context context,String content) throws WriterException {  
	 Bitmap ruseltBitmap = null;
		int marginW = 20;
	   // 生成一维条码,编码时指定大小,不要生成了图片以后再进行缩放,这样会模糊导致识别失败  
           BitMatrix matrix = new MultiFormatWriter().encode(content,  
             BarcodeFormat.CODE_128, dip2px(context,400),dip2px(context,100));  
	        int width = matrix.getWidth();  
	        int height = matrix.getHeight();  
	        int[] pixels = new int[width * height];  
	        for (int y = 0; y < height; y++) {  
	        ...
	        int w=dip2px(context,400);
	        int h=dip2px(context,100);
	        Bitmap bitmap = Bitmap.createBitmap(width, height,  
	                Bitmap.Config.ARGB_4444);  
	        // 通过像素数组生成bitmap,具体参考api  
	        bitmap.setPixels(pixels, 0, width, 0, 0, width, height);  
	        
	        Bitmap codeBitmap = creatCodeBitmap(content, width + 2
					* marginW, height, context);
